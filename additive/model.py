...
The model is everything the synth needs to play a timbre at any pitch:
  ...
  - inharmonicity B: partial k sits at k*f0*sqrt(1 + B*k^2).

File format (.addm) is deliberately trivial to parse from C++ (JUCE):
...
class AdditiveModel:
    ...
    def partial_freqs(self, f0: float) -> np.ndarray:
        ...
        return k * f0 * np.sqrt(1.0 + self.inharmonicity * k * k)
